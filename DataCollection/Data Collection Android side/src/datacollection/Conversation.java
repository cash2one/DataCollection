...
public class Conversation
{
	...
	public Conversation(JSONObject conversationJSON)
	{
		...
		{
			Log.e("JSON Error", "Conversation JSON not correctly formatted", e);
		}
	}
...
String fqlQuery = "SELECT message_id, thread_id, author_id, body, created_time, " + 
		...
		" ORDER BY created_time DESC LIMIT 40 OFFSET " + offset;
			...
			try
			{
				...
				else
				{
					...
					newMessagesLoaded = handleMessageQueryResponse(response, 
																   lastUpdatedTime,
			...
			{
				Log.e("JSON Error", "Error in handleMessageQueryResponse", e);
			}
		...
		Log.i("", "Updated with " + messages.size() + " new messages");
	}
	 ...
	 */
	private boolean handleMessageQueryResponse(Response response,
											   ...
											   int startIndex) throws JSONException
	{
		...
		JSONArray messageArray = response.getGraphObject()
	...
	public JSONObject getJSONRepresentation()
	{
		...
		{
			Log.e("JSON Error", "Error creating conversation JSON representation", e);
		};
		return result;
	}
