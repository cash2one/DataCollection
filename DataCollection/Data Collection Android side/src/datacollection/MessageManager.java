...
public class MessageManager
{
	...
	/**
	 * This creates a MessageManager, which is used for doing all operations
	 * on conversations.
	 * @param conversations old data that was loaded from JSON file. If it is
	 * null it isn't used.  
	 */
	...
	/**
	 * /**
	 * This method is used to collect all of the conversations.
	 * It does this using an FQL query. It collects 40 messages, then changes
	 * the offset by 40, gets 40 more, and so on until all conversations are collected.
	 ...
	 * @param limitToOneMonth whether to only load conversations that have
	 * taken place within the last month
	 * @param session The facebook session from which FQL requests can be sent
	 */
	public void loadConversations(boolean limitToOneMonth,
								  Session session)
	{
		int offset = 0;
		...
		"ORDER BY updated_time DESC LIMIT 40 OFFSET " + offset;
			...
			Request request = new Request(session, //Facebook session
					  ...
					  params, //Bundled parameters, mapped "q" and the query string
			...
			try
			{
				Response response = Request.executeBatchAsync(request).get().get(0);
				...
				if (response.getError() != null)
				{
					if (response.getError().getErrorCode() == RATE_LIMIT_EXCEEDED_ERROR)
					{
						...
						Log.i("INFO", "Waiting 3 minutes before trying again, we have exceeded the API rate limit");
						errorOccurred = true;
				...
				else
				{
					...
					JSONArray jsonConvos = response.getGraphObject().getInnerJSONObject().getJSONArray("data");
					...
					if (oldDataExists)
					{
						...
						//new data carefully
						conversationsLoaded = updateOldData(jsonConvos,
															session, 
															limitToOneMonth);
					}
					else
					{
						...
						for (int i = 0; i < jsonConvos.length(); i++)
						{
							...
							if (limitToOneMonth)
							{
								...
								if (todaysData.getTime().getTime() - c.getUpdatedTime().getTime().getTime() > MILLISECONDS_IN_DAY * 30)
								{
									conversationsLoaded = true;
									break;
								}
	 ...
	 * conversations list.
	 * @param jsonConvos The JSON data that holds a set of 40 conversations
	 * @param session The Facebook session that allows for FQL requests
	 * @param limitToOneMonth Boolean determining if conversations older than
	 * one month should be loaded
	 * @return If all conversations are loaded
	 * @throws JSONException
	 */
	private boolean updateOldData(JSONArray jsonConvos, 
								  Session session, 
								  boolean limitToOneMonth) throws JSONException
	{
		...
		for (int i = 0; i < jsonConvos.length() && !conversationsLoaded; i++)
		{
			...
			//In conversations the newest messages are at the front of the list.
			for (int k = 0; k < conversations.size(); k++)
			{
				
				//Our selected conversation is newer than the existing conversation
				if (c.getUpdatedTime().after(conversations.get(k).getUpdatedTime()))
				{
					...
					//loaded c's messages, we give it all of the k'th conversation's
					//messages.
					if (c.getId().equals(conversations.get(k).getId()))
					{
						oldUpdatedTime = conversations.get(k).getUpdatedTime();
						
						//Old conversation is now what c replaced in conversations
						oldConversation = conversations.set(k, c);
						
						//We give the old messages
						conversations.get(k).setMessages(oldConversation.getMessages());
						
						//We use the old latest updated time to set a baseline
						//for loading the new messages of the conversation.
						conversations.get(k).loadMessages(oldUpdatedTime, session, limitToOneMonth);
						
						//break out of the inner loop, back to a new JSON conversation
						break;
					}
					//We've found the correct place chronologically in the list
					//for our new conversation, so we put it in. We then go through
					//the rest of the list to remove any conversation with the same
					//thread ID. If there was one, we do the same thing as above,
					...
					else
					{
						...
						//Loop through and remove any conversation with the same ID
						for (int j = k + 1; j < conversations.size();)
						{
							if (conversations.get(j).getId().equals(c.getId()))
							{
								...
								oldUpdatedTime = oldConversation.getUpdatedTime();
							}
							else
								j++;
						}
				...
				//This can only happen if the previous condition evaluated to false
				//c.getUpdatedTime().after(conversations.get(k).getUpdatedTime())
				//and so the timestamps must be equal, therefore there are no updates.
				//If this happens, all later conversation in our JSON list will be
				...
				{
					Log.i("INFO", "DOES THIS EVER HAPPEN");
					c.loadMessages(null, session, limitToOneMonth);
					conversations.add(c);
				}
			}
