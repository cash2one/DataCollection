 ...
 * using a {@link com.example.messagesiphon.MessageManager} and a {@link com.example.messagesiphon.StreamManager}.
 * @author Tom
 *
 */
public class DataManager
{
	 ...
	 * data collection
	 * @param accessToken the string from the app that allows for Facebook access
	 */
	...
	public void loadOldJSONData()
	{
        ...
        {           
            ...
            System.out.println("******* File not found. Disregard this if you haven't run the program before.");
        } catch (IOException e) {
            e.printStackTrace();
        }
		catch (JSONException e)
	...
	public void collectParticipantInformation()
	{
		...
		try
		{
			...
			for (String idKey : result.keySet())
			{
				...
				{
					idNameMatches.put(idKey, result.get(idKey).getJSONObject(0).getString("name"));
				}
				else
				{
					System.out.println("Unable to get results for user id " + idKey);
				}
			}
		}
