 ...
 * conversation between two or more people. It also loads its messages using a
 * Facebook session and FQL requests.
 * 
 * @author Tom
 * 
 */
public class Conversation
{
	 ...
	 * with the oldest message at index 0, and the newest message at index (size
	 * - 1)
	 */
	...
	/**
	 * The string that holds the most recent message in a conversation. It is
	 * also found in {@messages} at the end of the list.
	 */
	...
	/**
	 * This creates a conversation object that holds all of the information in a
	 * Facebook conversation between two or more users.
	 * 
	 * @param conversationJSON
	 *            either the FQL JSON results or previously loaded conversation
	 *            JSON object
	 */
	public Conversation(JSONObject conversationJSON)
	{
		...
		try
		{
			...
			updatedTime.setTimeInMillis(conversationJSON
					.getLong("updated_time") * 1000);
	...
	/**
	 * This method is used to collect all of the messages for a conversation. It
	 * does this using an FQL query. It collects 40 messages, then changes the
	 * offset by 40, gets 40 more, and so on until all messages are collected.
	 ...
	 *            The time object that this conversation was last updated. It is
	 *            passed in if this conversation was previously on record,
	 *            otherwise it is null.
	 * @param session
	 *            The Facebook session used to send an FQL query
			...
			throws FacebookTokenExpiredError, FacebookUnhandledException
	{
		...
		while (!newMessagesLoaded)
		{
			...
			String fqlQuery = "SELECT message_id, thread_id, author_id, body, created_time, "
					+ "attachment FROM message WHERE thread_id="
					+ id
			...
			try
			{
				...
				newMessagesLoaded = handleMessageQueryResponse(results,
						lastUpdatedTime, restrictToOneMonthBack, startIndex);
			}
	...
	/**
	 * This method handles inserting the JSON data from the FQL query into the
	 * messages array list.
	 * 
	 * @param response
	 *            The FQL query results
	 * @param lastUpdatedTime
	 *            The same value as passed to loadMessages
	 * @param restrictToOneMonthBack
	 *            The same value as passed to loadMessages
	 * @param startIndex
	 *            The size of the messages ArrayList before any updates
	 * @return newMessagesLoaded - whether to continue updating.
	 * @throws JSONException
	 */
	private boolean handleMessageQueryResponse(JSONArray results,
			GregorianCalendar lastUpdatedTime, boolean restrictToOneMonthBack,
			int startIndex) throws JSONException
	{
		...
		for (int i = 0; i < messageArray.length() && !newMessagesLoaded; i++)
		{
			...
			{
				if (!(jsonMessage.getLong("author_id") + "")
						.equals(participants[j].getFacebookId()))
			...
			{
				// If the timestamp is more than 30 days old and we're
				// restricting
				// it to one month, finish loading messages.
				if (todaysDate.getTime().getTime()
						- message.getTimestamp().getTime().getTime() > MILLISECONDS_IN_DAY * 30)
					newMessagesLoaded = true;
			}
			...
			if (lastUpdatedTime == null
					|| message.getTimestamp().after(lastUpdatedTime))
			{
				/*
				 * The messages returned by FQL are in order from newest to
				 * oldest, so we put messages at the old length index so that we
				 * keep the oldest->newest order in our list.
				 * 
				 * 0 1 2 3 4 5 6 Old list: Z Y X W V U T startIndex = 7
				 * 
				 * FQL results: A B C inserted at position 7 list: Z Y X W V U T
				 * A list: Z Y X W V U T B A list: Z Y X W V U T C B A Time
				 * order was preserved.
				 */
	...
	/**
	 * @return The string representation of the conversation object, the thread
	 *         ID.
	 */
